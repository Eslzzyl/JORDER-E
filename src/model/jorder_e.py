...
class ResBlock(nn.Module):
    ...
    def forward(self, x, prev_x, is_the_second):
        if is_the_second == 1:
            x = x + self.relu(self.conv2(self.relu(self.conv1(x)))) + 0.1 * self.relu(
                self.conv4(self.relu(self.conv3(x)))) + self.relu(self.conv6(self.relu(self.conv5(x)))) * 0.1 + prev_x
        else:
...
class JORDER(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = nn.Conv2d(
            6, G0, kSize, padding=(kSize - 1) // 2, stride=1)
        self.updater = ResNet(G0, 9)

        self.mask_estimator1 = nn.Conv2d(
            G0, 8, kSize, padding=(kSize - 1) // 2, stride=1)
        self.mask_estimator2 = nn.Conv2d(
            8, 2, kSize, padding=(kSize - 1) // 2, stride=1)

        self.level_estimator1 = nn.Conv2d(
            G0, 8, kSize, padding=(kSize - 1) // 2, stride=1)
        self.level_estimator2 = nn.Conv2d(
            8, 3, kSize, padding=(kSize - 1) // 2, stride=1)

        self.mask_F_w_encoder1 = nn.Conv2d(
            2, 16, kSize, padding=(kSize - 1) // 2, stride=1)
        self.mask_F_w_encoder2 = nn.Conv2d(
            16, G0, kSize, padding=(kSize - 1) // 2, stride=1)

        self.mask_F_b_encoder1 = nn.Conv2d(
            2, 16, kSize, padding=(kSize - 1) // 2, stride=1)
        self.mask_F_b_encoder2 = nn.Conv2d(
            16, G0, kSize, padding=(kSize - 1) // 2, stride=1)

        self.level_F_w_encoder1 = nn.Conv2d(
            3, 16, kSize, padding=(kSize - 1) // 2, stride=1)
        self.level_F_w_encoder2 = nn.Conv2d(
            16, G0, kSize, padding=(kSize - 1) // 2, stride=1)

        self.level_F_b_encoder1 = nn.Conv2d(
            3, 16, kSize, padding=(kSize - 1) // 2, stride=1)
        self.level_F_b_encoder2 = nn.Conv2d(
            16, G0, kSize, padding=(kSize - 1) // 2, stride=1)
    ...
    def forward(self, x, x_prev, feat_pre, is_the_second, x_mask_prev, x_level_prev):
        ...
        if is_the_second == 1:
            x_mask = self.mask_estimator2(
                self.mask_estimator1(x_F)) + x_mask_prev
            x_level = self.level_estimator2(
                self.level_estimator1(x_F)) + x_level_prev
        else:
...
class JORDER_E(nn.Module):
    ...
    def forward(self, x):
        ...
        x2, feat_2, x_mask2, x_level2 = self.jorder2(
            x, x1, feat_1, 1, x_mask1, x_level1)
